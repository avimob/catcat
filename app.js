...
function renderCatalog(properties) {
  ...
  properties.forEach((property, index) => {
    ...
    const whatsappAmandaButton = node.querySelector(".whatsapp-amanda");
    const whatsappJoaoButton = node.querySelector(".whatsapp-joao");
    const whatsappAmandaUrl = buildAgentWhatsAppLink(property, WHATSAPP_AGENTS[0]);
    const whatsappJoaoUrl = buildAgentWhatsAppLink(property, WHATSAPP_AGENTS[1]);

    setWhatsAppButtonState(whatsappAmandaButton, whatsappAmandaUrl);
    setWhatsAppButtonState(whatsappJoaoButton, whatsappJoaoUrl);
...
function buildAgentWhatsAppLink(property, agent) {
  const phone = String(agent?.phone || "").replace(/\D/g, "");
  if (!phone) return "";

  const message = [
    `Ola, ${agent.name}! Tenho interesse no imovel:`,
    `${property.title}`,
...
function setWhatsAppButtonState(button, url) {
  if (!button) return;
